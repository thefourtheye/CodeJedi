...
class SubmitProblems:
    ...
    def POST(self):
        ...
        if Message != "":
            ...
            Problem     = Context.DB.find(dict(problemname=ProblemName), "Problems")
            if (len(Problem) == 1):
                return Templates.Problems.Submit("<li>Problem already exists with ID : "
                    + str(Problem[0].getProblemID()) + "</li>", True);
            Problem = Object(dict
                                (
            ...
            return Templates.Problems.Submit ("Problem Submitted Successfully with ID : " + str(ProblemID), False)
